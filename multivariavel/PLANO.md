...
- Seeds: 3 por (config treinável, horizonte) = **27 treinos** (3 configs: DLinear, CI, CD × 3 H × 3 seeds). Backbone/hiperparâmetros/early stopping verbatim do 14/15 por (modelo, seed), exceto camada de fusão multicanal + heads (novas, a especificar no README do M1). Execução nesta máquina (20c/117GB + 2×GPU 20GB, torch CUDA): M1 solo (valida pipeline); M2+M3 em paralelo, 1 job por GPU (`CUDA_VISIBLE_DEVICES=0/1`), threads capadas (`OMP/MKL/OpenBLAS_NUM_THREADS=8`). `DEVICE` parametrizável nos notebooks (`cuda` com fallback `cpu`).
...
- Custo ~5–6h CPU solo (27 treinos); com 2×GPU em paralelo cai p/ ~1–2h parede (DLinear = segundos; PatchTST acelera 5–10×). Nunca 2 jobs na mesma GPU.
