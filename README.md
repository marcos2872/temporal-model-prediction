...
- [x] Criar `notebooks/10-v2-baseline-ph.ipynb` (baselines no pH 2024 em protocolo v2: L=2304, purge/embargo, 5 fatias incl. dez — executado em servidor remoto; **RÉGUA v2: sazonal-naive 0,0406**; ARIMA com fallback total nos dias-âncora; Prophet rodou e colapsa 0,1463; artefatos em `resultados/10-v2-baseline-ph/`)
- [x] Criar `notebooks/11-v2-baseline-od.ipynb` (baselines no OD 2024 em protocolo v2: L=2304, purge/embargo, 5 fatias incl. dez — executado em servidor remoto; **RÉGUA v2: sazonal-naive 0,1736**; ARIMA com fallback total nos dias-âncora; Prophet rodou e colapsa 0,5243; artefatos em `resultados/11-v2-baseline-od/`)
- [x] Criar `notebooks/12-v2-lstnet-ph.ipynb` (LSTNet no pH 2024 em protocolo v2: L=2304, purge/embargo, 5 fatias, conv 3→8 solar+Fourier, 5 seeds — executado em servidor remoto; **0,0365±0,0004, −10,1% sobre o sazonal v2**; as 5 seeds batem o sazonal; artefatos em `resultados/12-v2-lstnet-ph/`)
- [x] Criar `notebooks/13-v2-lstnet-od.ipynb` (LSTNet no OD 2024 em protocolo v2: L=2304, purge/embargo, 5 fatias, conv 3→8 solar+Fourier, 5 seeds — executado em servidor remoto; **0,1467±0,0017, −15,5% sobre o sazonal v2**; as 5 seeds batem o sazonal; artefatos em `resultados/13-v2-lstnet-od/`)
- [x] Criar `notebooks/14-v2-patchtst-ph.ipynb` (PatchTST + DLinear no pH 2024 em protocolo v2: L=2304, purge/embargo, 5 fatias, 5 seeds por modelo — executado em servidor remoto; **patchtst 0,0352±0,0003 · dlinear 0,0356±0,0001, ambos abaixo do lstnet v2 e do sazonal v2**; overfit do v1 curado; artefatos em `resultados/14-v2-patchtst-ph/`)
- [x] Criar `notebooks/15-v2-patchtst-od.ipynb` (PatchTST + DLinear no OD 2024 em protocolo v2: L=2304, purge/embargo, 5 fatias, 5 seeds por modelo — executado em servidor remoto; **dlinear 0,1482±0,0027 · patchtst 0,1556±0,0028, dlinear empata o lstnet v2 e ambos batem o sazonal v2**; ordem do v1 mantida; artefatos em `resultados/15-v2-patchtst-od/`)
- [x] Criar `notebooks/16-v2-ensemble-ph.ipynb` (ensemble v2 no pH 2024 em protocolo v2: sazonal + LSTNet-12 + LGBM-nativo + DLinear-res + NNLS fit/report — executado em servidor remoto; **honesto (dez): ens 0,0324, dlres 0,0302**; in-sample fit 1–4: ens 0,0346; LGBM com peso não-zero; artefatos em `resultados/16-v2-ensemble-ph/`)
- [x] Criar `notebooks/17-v2-ensemble-od.ipynb` (ensemble v2 no OD 2024 em protocolo v2: sazonal + LSTNet-13 + LGBM-nativo + DLinear-res + NNLS fit/report — executado em servidor remoto; **honesto (dez): ens 0,1733, dlres 0,1842 ≈ lstnet 0,1848**; in-sample fit 1–4: ens 0,1234; LGBM zerado, padrão igual ao v1-07; artefatos em `resultados/17-v2-ensemble-od/`)
- [x] Criar `notebooks/18-v2-benchmark-2025.ipynb` (campeões v2 × 2025 intocado + lag-365 em protocolo v2, só inferência em servidor remoto; **pH patchtst 0,0465 (ens 0,0470); OD patchtst 0,2056 (ens 0,2133)**; ensemble não vence em 2025; lag-365/Prophet fora; artefatos em `resultados/18-v2-benchmark-2025/`)
