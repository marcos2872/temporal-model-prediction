...
NÃO lê `resultados/08-benchmark-2025/`. Carrega os checkpoints de `resultados/`
(00–07) e roda inferência (só inferência, sem treino/tuning) sobre 4 períodos
...
- PatchTST, base_feats, hour_sincos, lag365, prophet: cópia fiel de
  `notebooks/08-benchmark-2025.ipynb` cels 7/9/11 (pin: commit 97bceb0).

Uso:
    .venv/bin/python benchmark/benchmark-2025/benchmark_versoes.py --tag v1
    .venv/bin/python benchmark/benchmark-2025/benchmark_versoes.py --tag v1 --refazer
    .venv/bin/python benchmark/benchmark-2025/benchmark_versoes.py --tag v2
    .venv/bin/python benchmark/benchmark-2025/benchmark_versoes.py --diff v1 v2
    .venv/bin/python benchmark/benchmark-2025/benchmark_versoes.py --plot v1
    .venv/bin/python benchmark/benchmark-2025/benchmark_versoes.py --plot v1 v2

Saídas (tudo sob benchmark/benchmark-2025/resultados/):
...
SRC08 = ROOT / "resultados" / "08-benchmark-2025"  # só p/ checar; nada é lido de lá
...
def load_state(cls, rel, **kw):
    m = cls(**kw).to("cpu")
    m.load_state_dict(torch.load(ROOT / "resultados" / rel,
...
def load_lgbm(var: str):
    p = ROOT / "resultados" / CKPTS[var]["lgbm"]
    fb = ROOT / "resultados" / CKPTS[var]["lgbm_fb"]
